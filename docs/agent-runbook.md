...
> **Known cosmetic:** when two clients share one `--cdp` Chrome (site-docs `capture-auth` + browxai's `browxai-attached`), the browser console can throw `Function "__siteDocs_capture" is not exposed` / `Function "__browx_send" is not exposed` after one client disconnects but leaves its injected helper on the page. **Capture and discovery still complete; the cookie cache is valid.** Joint cleanup is tracked as an open ask on both sides — see the 2026-05-15 Codex adoption report for the details. Ignore the console noise for now.
