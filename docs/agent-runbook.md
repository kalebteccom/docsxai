...
**Reading a snapshot.** Header shows `url:`, `title:`, `stats:` (`a11yInteractive`, `domWalkEntries`, `domWalkNew`, `domWalkCombined`), and a `warnings:` block when the a11y tree was sparse (< 5 interactive descendants under root) explaining the source mix. Body lines: `role "name" [ref=eN] [<test-attr>="…"] [from-dom|from-both] [state]`. `[from-dom]` = DOM walk only (expected on heavy SPAs; act on the ref normally); `[from-both]` = both sources agreed (a good sign).

**Adoption status.** First adoption ran 2026-05-13 against the target app (modest win — orchestration solid; `find()` blunted by the then-unshipped DOM-walk fallback). Phase-1.5 fixes (DOM walk, `BROWX_TEST_ATTRIBUTES`, tier-1-no-role-gate, low-content warning) shipped same day. A **re-adoption run** that exercises `find()` against the augmented snapshot is the remaining gate on both browxai Phase 1 and this integration contract; if tier-3/4 selectorHints (stable-text-on-stable-role, id/semantic — currently degrade to tier-5 `role=<role>` with `stability: low`) come up missing on a step that should have had them, file it on the browxai side rather than working around it. Full contract + deferrals at [`docs/browxai-asks.md`](browxai-asks.md).
