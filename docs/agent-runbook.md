...
> **Shared-CDP page-helper lifecycle (site-docs side fixed 2026-05-15):** when two clients share one `--cdp` Chrome (site-docs `capture-auth` + browxai's `browxai-attached`) and one disconnects, the page-side helpers it injected become stale. Site-docs's `window.__siteDocs.capture()` now detects the detached binding on next invocation, removes its injected button if any, logs `[site-docs] capture helper detached…`, and self-deletes from `window.__siteDocs`. Reload the page or re-run `capture-auth` to install a fresh helper. Browxai's `__browx_send` may still surface a similar cosmetic error per the Codex adoption report — track upstream.
...
steps:
  ...
  - id: dismiss-confirm-modal
    ...
    wait_for: { selector: $modal_ok }            # For conditionally-present UI (a confirm modal that sometimes appears, a first-run tooltip, a cookie banner).
