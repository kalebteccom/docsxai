...
Per call: ≤ a few hundred milliseconds total. Site-docs's `actionable()` takes a `timeoutMs` (default 300 ms) that bounds the *per-check* probe — keep it small enough that calibration-time discovery doesn't stall.
...
- **The state is a *snapshot*.** Repeatedly calling `actionable()` after each action is reasonable; relying on a cached value across an action is not.
- **`multiple-matches` is the round-6 hidden-duplicate signal.** If a `[data-foo="x"]` is `multiple-matches` and only one is visible, the calibration agent's prescribed move is to emit `[data-foo="x"]:visible` (or equivalent) into the flow-file — *not* to silently pick one. Same fix the runbook's "Locator gotchas" block names.

## Coordinates with the existing halt-cause prefix

The runtime's halt-cause prefix (see `flow-runtime.ts: inferHaltCause`) parses Playwright actionability *errors* into the same vocabulary at halt-time. Pre- and post-action signals should match:
