...
export WORKSPACE=$HOME/site-docs/$(basename "$APP_REPO")   # site-docs artifacts go HERE, never in $APP_REPO
export APP_RUN=/tmp/site-docs-app-run                       # disposable copy of $APP_REPO to actually run
```
...
### 2b. Create the site-docs workspace (outside everything)
...
you can click through); when you're in, run `window.__siteDocs.capture()` in the devtools console (or click the
injected button):

```bash
site-docs capture-auth "$WORKSPACE" --base-url "$APP_URL" --ignore-https-errors
...
are built this becomes `/site-docs:calibrate <flow.md> --url "$APP_URL" --into "$WORKSPACE"`.)

### 2e. Re-run deterministically (refresh the docs)

```bash
site-docs run "$WORKSPACE" --base-url "$APP_URL" --ignore-https-errors
# headless Chromium, uses $WORKSPACE/.auth/editor.json, replays $WORKSPACE/flows/*.flow.yaml,
# re-emits $WORKSPACE/docs/<flow>/annotations.json + screenshots. Halts on drift — see /site-docs:diagnose.
```

### 2f. View

```bash
site-docs render "$WORKSPACE"          # → $WORKSPACE/.viewer/index.html
