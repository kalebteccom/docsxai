...
1. The command reads the workspace config (`.site-docs.json` → `plugins`, `plugin_capabilities`).
2. Each source is resolved to a manifest: `{ package }` via Node module resolution, `{ path }` via realpath'd filesystem path.
3. If `plugins-lock.json` exists next to the workspace config, every plugin's register-module bytes are sha256-verified **before** import. Mismatch → that plugin load-errors with a "lock mismatch — run `site-docs plugins sync`" message.
...
Every plugin declares a mandatory `namespace` (`/^[a-z][a-z0-9-]*$/`, kebab-case). Every artifact the plugin registers is exposed as `<namespace>:<name>` — the runtime applies the prefix; plugins pass bare names. Reserved namespaces: `site-docs`, `docsxai`, `core`, `plugins`. Claiming one is a manifest-validation failure.
...
A plugin declares the capabilities its artifacts need in the manifest — today the only capability family is `egress:<host-glob>` (e.g. `egress:*.atlassian.net`); unknown prefixes are rejected at manifest validation. The declared set is subset-checked (exact string match) against the operator-enabled set (`plugin_capabilities` in `.site-docs.json`). A mismatch disables the plugin with `disabled-by-capability-mismatch` — a status, not a fatal error.
...
`plugins-lock.json` pins the sha256 of each plugin's register-module bytes; resolution verifies the hash before importing, so a silently-swapped module fails closed. `site-docs plugins sync` (re)writes the lock.
