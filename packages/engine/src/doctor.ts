...
export async function probeChromium(): Promise<{ ok: boolean; detail: string }> {
  try {
    // Lazy import so doctor doesn't pay the playwright-core cost before this row.
    const { chromium } = await import("playwright-core");
    const p = chromium.executablePath();
    if (p && existsSync(p)) return { ok: true, detail: p };
...
/**
 * The plugins rows mirror `docsxai plugins list` statuses through the same stage-1
 * resolution + lock verification + capability subset check the runtime runs — but doctor
 * NEVER imports a plugin's register module. Executing plugin code is the CLI command's job.
 */
