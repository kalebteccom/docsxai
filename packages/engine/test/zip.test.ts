...
async function makeWorkspace(): Promise<string> {
  ...
  await fs.writeFile(path.join(tmp, "flows", "f1.flow.yaml"), "name: f1\nsteps:\n  - id: s\n    action: wait\n", "utf8");
  ...
  await fs.writeFile(path.join(tmp, "auth", "strategy.yaml"), "schema: site-docs/auth-strategy@1\n", "utf8");
...
describe.skipIf(!zipAvailable)("zipDocPack", () => {
  ...
  it.skipIf(!unzipAvailable)("includes flows/, docs/ (minus halts/), .site-docs.json, auth/strategy.yaml", async () => {
    const out = path.join(outDir, "pack.zip");
  ...
  it.skipIf(!unzipAvailable)("excludes .viewer/ by default; --include-viewer pulls it in", async () => {
  ...
  it("throws ZipError on a non-existent workspace", async () => {
    await expect(zipDocPack({ workspace: "/definitely/not/real", output: path.join(outDir, "x.zip") })).rejects.toThrow(ZipError);
  });
  ...
  it("throws ZipError on a workspace with nothing to zip", async () => {
    ...
    await expect(zipDocPack({ workspace: empty, output: path.join(outDir, "x.zip") })).rejects.toThrow(/nothing to zip/);
