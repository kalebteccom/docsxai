...
Drive the operator's live browser through a **host-agent-drivable browser API over MCP** (the BYOB pattern: the engineer is already authenticated to the target site there — never ask for credentials, never put credentials anywhere). The canonical, model-agnostic driver is **[browxai](https://github.com/kalebteccom/browxai)** — Kalebtec's MCP-native browser bridge — which exposes a curated surface (`find(query)` returning ranked candidate locators with evidence, `snapshot()` with persistent refs, action primitives that report what changed). It's attach-friendly: aim it at the `--cdp` Chrome from `site-docs capture-auth` and the same authed session drives discovery without a second login. **Caveat:** browxai's Phase-0 spike doesn't yet do CDP-attach against an authed host Chrome; the concrete capability asks live at `docs/browxai-asks.md` in the engine repo. Until those land, fall back to **`site-docs inspect "$WORKSPACE" --cdp <endpoint>`** or driving the `--cdp` Chrome with Playwright directly (`chromium.connectOverCDP(...)`). *Legacy:* Anthropic's **Claude in Chrome** extension (`mcp__claude-in-chrome__*`) is still a valid driver in a Claude-Code-driven session, but it's Claude-locked and no longer the canonical path. For each flow in the description:
