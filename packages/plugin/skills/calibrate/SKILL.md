...
Drive the operator's live browser through a **host-agent-drivable browser API over MCP** (the BYOB pattern: the engineer is already authenticated to the target site there — never ask for credentials, never put credentials anywhere). The canonical, model-agnostic driver is **[browxai](https://github.com/kalebteccom/browxai)** — Kalebtec's MCP-native browser bridge. Phase 1 has shipped: `pnpm browxai` (or the `browxai` bin) starts the MCP server; `BROWX_ATTACH_CDP=http://localhost:9222` attaches it to the `--cdp` Chrome `site-docs capture-auth` already runs, so the authed session drives discovery without a second login. The host agent gets `find(query)` (ranked candidate locators with `selectorHint` + `stability: high|medium|low` + visible-rect bbox + evidence), `snapshot()` (compact a11y tree with refs that persist within a session), action primitives that return what changed, `await_human({kind:"acknowledge"})` for human checkpoints, plus screenshots / console / network reads. Full Phase-1 contract + what's deferred to Phase-1.5 (`selectorHint` tiers 3-4) and Phase 2 (`dump_storage_state`, scoped `snapshotDelta`, more `await_human` kinds): `docs/browxai-asks.md` in the engine repo. Fallbacks if browxai misbehaves on a specific page: **`site-docs inspect "$WORKSPACE" --cdp <endpoint>`** or driving the same `--cdp` Chrome with Playwright directly (`chromium.connectOverCDP(...)`). *Legacy:* Anthropic's **Claude in Chrome** extension (`mcp__claude-in-chrome__*`) is still a valid driver in a Claude-Code-driven session, but it's Claude-locked and isn't the canonical path. For each flow in the description:
