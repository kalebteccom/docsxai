...
no Chromium binary, tell the user to run `npx playwright-core install chromium` (source checkout:
`pnpm -C packages/engine exec playwright-core install chromium`). If it reports an expired/missing
