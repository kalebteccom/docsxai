...
describe("placeCallout", () => {
  it("uses the preferred side when it fits", () => {
    const p = placeCallout({ image, target: { x: 400, y: 400, width: 40, height: 20 }, callout, preferred: "top" });
  ...
  it("falls off the preferred side when it doesn't fit (target near the top → goes bottom)", () => {
    const p = placeCallout({ image, target: { x: 400, y: 5, width: 40, height: 20 }, callout, preferred: "top" });
  ...
  it("never lets the callout leave the image (clamped)", () => {
    const p = placeCallout({ image, target: { x: 5, y: 5, width: 10, height: 10 }, callout: { width: 400, height: 400 }, preferred: "top" });
  ...
  it("picks the side with the most room when nothing fits", () => {
    const p = placeCallout({ image, target: { x: 10, y: 10, width: 980, height: 600 }, callout: { width: 400, height: 300 }, preferred: "top" });
    // top room 10, bottom 190, right 10, left 10 → bottom
    expect(p.side).toBe("bottom");
  });

  it("respects a left/right preference", () => {
    const p = placeCallout({ image, target: { x: 600, y: 400, width: 40, height: 20 }, callout, preferred: "right" });
